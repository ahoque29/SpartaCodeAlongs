...
namespace Mock_Example_Tests
{
	...
	public class CatalogItemManagerShould
	{
		...
		[Test]
		public void GivenAnItemIsRemoved_FindItemReturnsNull()
		{
			// Remove existing item
			_sut.RemoveItem("Teaset");

			// Test that the item no longer exists
			Assert.That(_sut.FindItem("Teaset"), Is.Null);

			// Add back the item
			_sut.AddItem(new CatalogItem() { CatalogItemId = 1, Brand = "Acme", Name = "Teaset", Price = 20.00m });
		}
	}
}